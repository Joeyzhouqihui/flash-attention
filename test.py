...
def checkAttnScores(flash_attn_scores, torch_attn_scores, batch_size, num_head, query_len, key_len):
    diff = 0.0
    for batch in range(batch_size):
        for head in range(num_head):
            for row in range(query_len):
                end = key_len - query_len + 1
...
if __name__ == "__main__":
    ...
    checkAttnScores(flash_attn_scores, torch_attn_scores, batch_size, num_head, query_len, query_len + local_window - 1)
    # len_q = 3
    # len_k = 8
    # ws = 5

...
mha_fwd_kvcache_multiple(at::Tensor &old_q,                  // batch_size x seqlen_q x num_heads x head_size
    ...
    for (int iteration = 0; iteration < max_iterations; iteration++) {
        ...
        process_es(
            params,
            es_acc, es_min,
            total_seq_lens, block_chunk_size,
            seq_states, compute_iteration,
            iteration, threshold,
            stream
        );
        while (*compute_iteration == iteration);
        int num_finished_seqs = 0;
        for (int i = 0; i < batch_size; i++) {
            if (seq_states[i] == 0) {
                num_finished_seqs++;
            }
        }
        if (num_finished_seqs == batch_size) {
            break;
        }
    }
    printf("max iteration: %d, iteration: %d\n", max_iterations, (*compute_iteration));
    // int val;
    // std::cin>>val;
}